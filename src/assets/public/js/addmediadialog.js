class AddMediaDialog {
	constructor(player) {
		this.model = player;
		this.$el = $('<div class="modal fade">');
		this.$el.html(AddMediaDialog.template()).modal();

		this.$el.find('.do-add-netmedia').on('click', (event) => {
			var url = this.$el.find('.mediadialog-value-netmedia').val();
		...
		this.$el.find('.do-add-files').on('click', (event) => {
			var files = this.$el.find('.mediadialog-value-files')[0].files;
...
AddMediaDialog.template = _.template(`
